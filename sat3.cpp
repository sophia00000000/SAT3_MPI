...
void ejecutarManager(const ExperimentConfig& cfg)
{
    ...
    for (int rep = 0; rep < cfg.repetitions; rep++) {
        TaskPayload tareaWorker1 = {rep, 0};
        TaskPayload tareaWorker2 = {rep, 1};

        MPI_Send(&tareaWorker1, static_cast<int>(sizeof(TaskPayload)), MPI_BYTE,
                 1, TAG_TASK, MPI_COMM_WORLD);
        MPI_Send(&tareaWorker2, static_cast<int>(sizeof(TaskPayload)), MPI_BYTE,
                 2, TAG_TASK, MPI_COMM_WORLD);

        int satGlobal = 0;
        vector<WorkerReport> reportesRep;
        reportesRep.reserve(2);
    ...
    long long totalMessages = static_cast<long long>(cfg.repetitions) * 4 + 2;
    long long bytesTask = static_cast<long long>(cfg.repetitions) * 2 * sizeof(TaskPayload)
                       + 2 * sizeof(TaskPayload);
    long long bytesResult = static_cast<long long>(cfg.repetitions) * 2 * sizeof(WorkerReport);
...
int main(int argc, char** argv){
    ...
    if (size != 3){
        if (rank == 0) {
            cout << "Este ejemplo requiere exactamente 3 procesos: "
                 << "manager(0), worker1(1), worker2(2).\n";
        }
